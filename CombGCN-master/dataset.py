...
class BasicDataset(Dataset):
    ...
    def load_B_I_affiliation(self):
        with open(os.path.join(self.path, self.name, 'bundle_item.txt'), 'r') as f:
            return list(map(lambda s: tuple(int(i) for i in s[:-1].split('\t')), f.readlines()))#s[:-1]代表从位置0到位置-1之前的数


class BundleTrainDataset(BasicDataset):
    def __init__(self, path, name, item_data, assist_data, seed=None):
        super().__init__(path, name, 'train', 1)#负采样数量=1？
        # U-B
...
class AssistDataset(BasicDataset):
    def __init__(self, path, name):
        ...
        # B-I
        self.B_I_pairs = self.load_B_I_affiliation()#读取了bundle_item中的数据对
        indice = np.array(self.B_I_pairs, dtype=np.int32)#indice为数据创建了数组
        values = np.ones(len(self.B_I_pairs), dtype=np.float32)#self.B_I_pairs的第0维长度=472，一维的472个1
