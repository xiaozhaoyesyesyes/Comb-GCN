...
def graph_generating(raw_graph, row, col):#图的生成（没用上）
    if raw_graph.shape == (row, col):
        graph = sp.bmat([[sp.identity(raw_graph.shape[0]), raw_graph],
                             [raw_graph.T, sp.identity(raw_graph.shape[1])]])#sp.bmat返回二维矩阵，sp.identity返回稀疏格式的单位矩阵
    else:
        raise ValueError(r"raw_graph's shape is wrong")
    return graph

def laplace_transform(graph):#拉普拉斯变换
    rowsum_sqrt = sp.diags(1/(np.sqrt(graph.sum(axis=1).A.ravel()) + 1e-8))#稀疏矩阵对角线上的元素
...
def to_tensor(graph):
    graph = graph.tocoo()
    values = graph.data
    indices = np.vstack((graph.row, graph.col))#堆叠矩阵
...
class BGCN(Model):
    ...
    def __init__(self, info, dataset, raw_graph, device, pretrain=None):
        ...
        if ui_graph.shape == (self.num_users, self.num_items):
            ...
            atom_graph = sp.bmat([[sp.identity(ui_graph.shape[0]), ui_graph],
                                 [ui_graph.T, sp.identity(ui_graph.shape[1])]])#二维矩阵
        else:
            raise ValueError(r"raw_graph's shape is wrong")
        ...
        if ub_graph.shape == (self.num_users, self.num_bundles) \
            ...
            non_atom_graph = sp.bmat([[sp.identity(ub_graph.shape[0]), ub_graph],
                                 [ub_graph.T, bb_graph]])#sp.bmat()构建矩阵
        else:
            raise ValueError(r"raw_graph's shape is wrong")
    ...
    def one_propagate(self, graph, A_feature, B_feature, dnns):
        ...
        # propagate
        features = torch.cat((A_feature, B_feature), 0)#将两个张量竖着拼接到一起
    ...
    def propagate(self):
        ...
        atom_bundles_feature = F.normalize(torch.matmul(self.pooling_graph, atom_items_feature))#学习特征归一化torch.matmul为tensor乘法 输入可以是高维的
        ...
        # bundles_feature = atom_bundles_feature
